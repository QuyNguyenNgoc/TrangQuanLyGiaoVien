...
namespace Hinnova.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : HinnovaServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task TryToSendChatMessageMail(User user, string senderUsername, string senderTenancyName, ChatMessage chatMessage)
        {
            try
            {
                var emailTemplate = GetTitleAndSubTitle(user.TenantId, L("NewChatMessage.Email_Title"), L("NewChatMessage.Email_SubTitle"));
                var mailMessage = new StringBuilder();
                ...
                await ReplaceBodyAndSend(user.EmailAddress, L("NewChatMessage.Email_Subject"), emailTemplate, mailMessage);
            }
