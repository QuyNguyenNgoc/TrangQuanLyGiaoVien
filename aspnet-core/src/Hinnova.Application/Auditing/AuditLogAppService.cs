...
using Hinnova.Auditing.Exporting;
using Hinnova.Authorization;
using Hinnova.Authorization.Users;
using Hinnova.Dto;
using Hinnova.EntityHistory;
using EntityHistoryHelper = Hinnova.EntityHistory.EntityHistoryHelper;
using System;
...
namespace Hinnova.Auditing
{
    [DisableAuditing]
    [AbpAuthorize(AppPermissions.Pages_Administration_AuditLogs)]
    public class AuditLogAppService : HinnovaAppServiceBase, IAuditLogAppService
    {
        ...
        public async Task<QueryResult> ExecuteRawQueryAsync(string query)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    IEnumerable<dynamic> rows = conn.Query(query);
                    var queryResult = new QueryResult { Code = "", Data = new List<object>(), isSucceeded = true, Message = "OK" };
                    var resList = new List<DataVm>();
                    foreach (var item in rows)
                    {
                        queryResult.Data.Add(JsonConvert.SerializeObject(item));
                    }
                    ...
                    //}
                    //List<ExpandoObject> listObj = new List<ExpandoObject> { };
                    ...
                    //    {
                    //        //var prop = pair.Key;
                    //        //MyDynamic.Add(pair.Key, pair.Value);
                    //        ((IDictionary<string, object>)MyDynamic)[pair.Key] = pair.Value;
                    //        //var property = pair.Key;
                    //        //var value = pair.Value;
                    //        //MyDynamic.property = value;
                    //        //MyDynamic
                    //    }
                    //    listObj.Add(MyDynamic);
                    //    //var kkkk = MyDynamic;
                    //}
                }
            }
            catch (Exception ex)
            {
                ...
                return null;               
                //return DataVm.Fail("ERR", "Không truy vấn được từ store: " + "LCT_ScheduleApprove_UpdateXml");
            }
        }
