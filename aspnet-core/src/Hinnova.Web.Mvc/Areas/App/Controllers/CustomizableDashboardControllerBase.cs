...
using Hinnova.DashboardCustomization.Dto;
using Hinnova.Web.Areas.App.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : HinnovaControllerBase
    {
        ...
        public async Task<PartialViewResult> AddWidgetModal(string dashboardName, string pageId)
        {
            var userDashboard = await DashboardCustomizationAppService.GetUserDashboard(
                new GetDashboardInput
                {
                    ...
                    Application = HinnovaDashboardCustomizationConsts.Applications.Mvc
                }
            );
