...
import { CachingServiceProxy, EntityDtoOfString, WebLogServiceProxy, AuditLogServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class MaintenanceComponent extends AppComponentBase implements OnInit {
    ...
    getQueryResult(){
        this._auditLog.executeRawQuery(this.rawSql).subscribe(result => {
            if(result.data== undefined || result.data.length == 0 || result.data.length == null){
                this.notify.error("Câu lệnh sai hoặc không có dữ liệu!");
                return;
            }
            let firstRow = JSON.parse(result.data[0]);
            this.tableHeader = Object.getOwnPropertyNames(firstRow);
            result.data = result.data.map(x => JSON.parse(x));
